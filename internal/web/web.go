...
func SPAHandler(webDir string) app.HandlerFunc {
	...
	return func(ctx context.Context, c *app.RequestContext) {
		...
		// 前端未构建。
		log.Warn("前端未构建, 请先执行 make web-build 或 npm run build",
...
func EnsureDir(webDir string) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			log.Warn("WEB_DIR 目录不存在, 前端将走引导提示页", "dir", webDir)
			return nil
		}
		return err
	}
	if !info.IsDir() {
		log.Warn("WEB_DIR 不是目录", "dir", webDir)
	}
	return nil
}
