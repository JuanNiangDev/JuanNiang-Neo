...
func (p *Adapter) Start(ctx context.Context) error {
	...
	p.closed = false
	p.mu.Unlock()
	logging.Info("adapter 已启动", "addr", listenAddr)
	return nil
}
...
func (p *Adapter) Stop(ctx context.Context) error {
	...
	case err := <-done:
		return err
	case <-ctx.Done():
		logging.Warn("adapter Stop 超时, 强制退出", "err", ctx.Err())
		return ctx.Err()
	}
}
...
func (p *Adapter) Restart(ctx context.Context) error {
	...
	if err := p.Stop(ctx); err != nil {
		logging.Error("adapter 重启出错 (Stop)", "err", err.Error())
		return err
	}

	if err := p.Start(ctx); err != nil {
		logging.Error("adapter 重启出错 (Start)", "err", err.Error())
		return err
	}
...
func (p *Adapter) SyncConfig(ctx context.Context, conf Config) error {
	...
	//   - 停用: 调用 Stop
	if conf.Enable {
		logging.Info("adapter 重启中")
		if err := p.Stop(ctx); err != nil {
			logging.Error("adapter 配置更新出错 (Stop)", "err", err.Error())
			return err
		}
		if err := p.Start(ctx); err != nil {
			logging.Error("adapter 配置更新出错 (Start)", "err", err.Error())
			return err
		}
	} else {
		if err := p.Stop(ctx); err != nil {
			logging.Error("adapter 停止Adapter出错", "err", err.Error())
			return err
		}
	}

	logging.Info("adapter 配置已更新")
	return nil
}
