...
func (b *BackgroundTaskExecutor) executeTool(ctx context.Context, toolName string, args json.RawMessage) (string, error) {
	...
	if b.mcpGroup != nil {
		if b.mcpGroup.HasTool(ctx, toolName) {
			log.Info("BgTask 执行 MCP 工具", "tool", toolName)
			return b.mcpGroup.CallTool(ctx, toolName, args)
		}
	}
	// 回退到内置工具
	log.Info("BgTask 执行内置工具", "tool", toolName)
...
func (b *BackgroundTaskExecutor) executeAsync(task *models.BackgroundTask, msgType string, targetID int64, userPrompt string, steps []TaskStep) {
	...
	log.Info("后台任务执行开始", "task_id", task.ID, "steps", len(steps))
	for _, s := range steps {
		log.Info("后台任务步骤", "task_id", task.ID, "step_id", s.ID, "tool", s.ToolName)
	}
	...
	for len(completed) < len(steps) {
		...
		for _, s := range steps {
			...
			step := s
			g.Go(func() error {
				log.Info("后台任务步骤执行中", "task_id", task.ID, "step_id", step.ID, "tool", step.ToolName)
				result, err := b.executeTool(gCtx, step.ToolName, step.Args)
				mu.Lock()
				defer mu.Unlock()

				if err != nil {
					log.Error("后台任务步骤执行失败", "task_id", task.ID, "step_id", step.ID, "tool", step.ToolName, "err", err)
					results[step.ID] = "error: " + err.Error()
					stepErrors[step.ID] = err.Error()
					hasFailed = true
				} else {
					log.Info("后台任务步骤执行完成", "task_id", task.ID, "step_id", step.ID, "tool", step.ToolName, "result_len", len(result))
	...
	log.Info("后台任务执行完成", "task_id", task.ID, "status", finalStatus, "failed", hasFailed)
}

// Run 启动后台任务执行器，并恢复 DB 中未完成的任务。
func (b *BackgroundTaskExecutor) Run(ctx context.Context) {
	log.Info("BackgroundTaskExecutor 已启动")
...
func (b *BackgroundTaskExecutor) recoverTasks(ctx context.Context) {
	...
	if err != nil {
		log.Error("恢复后台任务失败: 查询 DB 出错", "err", err)
		return
	}
	if len(tasks) == 0 {
		return
	...
	log.Info("正在恢复未完成的后台任务", "count", len(tasks))
	for _, t := range tasks {
		// 解析 Steps JSON → []TaskStep
		stepsJSON, _ := json.Marshal(t.Steps)
		var steps []TaskStep
		if err := json.Unmarshal(stepsJSON, &steps); err != nil {
			log.Error("恢复任务失败: 解析步骤出错", "task_id", t.ID, "err", err)
		...
		if t.TargetID == 0 || t.MessageType == "" {
			log.Warn("跳过无法恢复的旧任务（缺少发送目标）", "task_id", t.ID)
			b.dao.UpdateStatus(ctx, t.ID, models.TaskStatusFailed)
			continue
		}
		log.Info("恢复执行后台任务", "task_id", t.ID, "status", t.Status)
