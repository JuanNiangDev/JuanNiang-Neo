...
const llmPhrasePrompt = `你是一个 QQ 大学新生群（迎新群）的内容审查员，负责把每条消息归类到「黑名单」或「白名单」。
...
判定标准：
1. 黑名单（black）：消息构成违规，需要处罚。包括：
   - 广告违规：推销办卡/套餐/流量卡、贷款/提额/套现、兼职/刷单/修图结算、寄宿自习室/考研考公机构、0元购/1折/送福利、以"学校通知/勤工实践/资料分享"为幌子拉人进群、留下微信号/QQ群号/二维码引流等。
   - 敏感违规：色情、软色情/擦边、政治敏感、暴力、赌博、辱骂/人身攻击/恶意诅咒（如"死妈""sb""傻逼"等侮辱性词汇）、代孕/器官买卖等违法违禁内容。软色情从严：性暗示 + 学生/校园语境（校服、课堂、新生）必须判 black，即使没有露骨词。
...
2. 白名单（white）：消息明确无害，值得直接放行并可作正常交流模板，如同学间的日常问候、约饭、游戏开黑、学习互助、班级通知等正常交流。
3. 无法确定（none）：语义模糊、无法判断是否违规，或消息过短无上下文。
...
- 辱骂/人身攻击/恶意诅咒、色情/擦边、赌博、代孕等违法违禁内容必须判 black。
- 尾缀 emoji（如"哈哈哈哈🤣""好的👌"）不构成违规；emoji 只有在能合理还原为营销/色情语义时才作为变体信号。

输出格式（批量逐条，严格 JSON，不要输出任何其它文字）：
{"results":[{"index":0,"verdict":"black|white|none","reason":"一句话说明理由"},{"index":1,...}]}
索引必须与输入块一一对应，每条只输出一个结果。`
