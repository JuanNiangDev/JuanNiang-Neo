...
var (
	pardonTemplates = struct {
		...
		ok: []string{
			...
			"收到！%d 解禁 + 违规清零完成，我会继续盯着 TA 的～",
			"%d 这次先放过：解除禁言、清空违规记录～下次再犯可就要按规矩来咯～",
		},
		whitelisted: []string{"%d 本来就在白名单里，不用豁免啦～我顺手帮 TA 解了禁言、清了记录～"},
		denied: []string{
			"只有管理员才能用豁免哦～",
			"豁免是管理员的专属技能啦，你不行哦～",
		},
	...
	whitelistTemplates = struct {
		...
		ok: []string{
			"好啦，%d 已经被我记到白名单小本本上啦，违规记录也清空咯～",
			"收到！%d 已加入白名单，以后可以放心发言，我不会管 TA 啦，违规记录已清空～",
			"%d 获得免死金牌一枚！我已将 TA 加入白名单，并清空了违规记录哦～",
		},
		denied: []string{
			"只有管理员才能用白名单哦～",
			"白名单是管理员的专属技能啦，你不行哦～",
