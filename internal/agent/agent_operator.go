...
func (h *HagoCenter) SwitchProvider(ctx context.Context, id string) error {
	...
	logging.Info("Provider 切换完成", "id", id, "type", p.Type)
	return nil
}
...
func (h *HagoCenter) SetToolActive(ctx context.Context, name string, active bool) error {
	...
	if !active {
		...
		logging.Info("Tool 已停用", "name", name)
	} else {
		...
		if _, ok := h.Tools.Get(name); !ok {
			...
			logging.Warn("Tool 重新启用需重启或重新注册", "name", name)
		}
	}
	return nil
}
...
func (h *HagoCenter) SetT2IActive(ctx context.Context, active bool) error {
	...
	if active {
		...
		h.T2IClient = client
		logging.Info("T2I 服务已启用", "base_url", cfg.BaseURL)
	} else {
		h.T2IClient = nil
		logging.Info("T2I 服务已停用")
	}
	return nil
}
...
func (h *HagoCenter) SetSandboxActive(ctx context.Context, active bool) error {
	...
	if active {
		...
		logging.Info("Sandbox 服务已启用", "base_url", cfg.BaseURL)
	} else {
		h.SandboxClient = nil
		logging.Info("Sandbox 服务已停用")
	}
	return nil
}
