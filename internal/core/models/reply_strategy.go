...
const (
	StrategyNeverReply ReplyStrategy = "never_reply" // 完全不回复（默认）
	StrategyAtOnly     ReplyStrategy = "at_only"     // 仅@我时回复
	StrategyAlways     ReplyStrategy = "always"      // 完全回复
...
type ReplyStrategyConfig struct {
	...
	Strategy           ReplyStrategy `gorm:"not null;default:'never_reply'" json:"strategy"`
