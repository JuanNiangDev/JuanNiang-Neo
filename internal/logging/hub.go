...
// Entry 表示一条日志记录，同时用于 stdio 输出和 Web UI 推送。
type Entry struct {
	...
	Stack   string         `json:"stack,omitempty"` // Warn 及以上级别包含调用栈
}

// Hub 是日志中心：维护最近 MaxBuffer 条日志的环形缓冲，并把新日志广播给订阅者。
//
// 进程内单例 DefaultHub 被 Logger 持有；HTTP/SSE 端从同一个 Hub 读取历史与订阅实时事件。
type Hub struct {
	mu          sync.RWMutex
	buffer      []Entry
	head        int
...
// Push 写入一条日志并广播给所有订阅者。慢订阅者（channel 满）会被跳过。
