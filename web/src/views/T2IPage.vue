...
import { t2iApi, type T2IConfigResp, type UpdateT2IConfigReq } from '@/api'
import { useToastStore } from '@/stores/toast'

const toastStore = useToastStore()
const saving = ref(false); const checking = ref(false)
const config = ref<T2IConfigResp>({ base_url: '', timeout: 120000, is_active: false, healthy: false })
const form = ref<UpdateT2IConfigReq>({ base_url: '', timeout: 120000, is_active: false })

async function fetchConfig() { try { const res = await t2iApi.getConfig(); config.value = res.data.data; form.value = { base_url: config.value.base_url, timeout: config.value.timeout, is_active: config.value.is_active } } catch { toastStore.error('获取配置失败') } }
async function save() { saving.value = true; try { const res = await t2iApi.updateConfig(form.value); config.value = res.data.data; toastStore.success('已保存') } catch { toastStore.error('保存失败') } finally { saving.value = false } }
async function checkHealth() { checking.value = true; try { const res = await t2iApi.health(); config.value.healthy = res.data.data.healthy; toastStore.info(config.value.healthy ? '健康检查通过' : '健康检查失败') } catch { toastStore.error('健康检查失败') } finally { checking.value = false } }
