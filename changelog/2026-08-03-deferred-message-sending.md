...
### 群聊 @ 回复规则
- 系统锁定提示词新增「群聊 @ 回复」规则：群聊中回复单条短消息（问问题/打招呼/接话）时，用 `[CQ:at,qq=发送者QQ]` 点名对方
- 长内容/图片海报/群公告不 @，避免刷屏；私聊无 @ 概念；@ 的 QQ 必须取自会话信息的「发送者QQ」
- 重启后 `EnsureSystemPrompt` 自动同步到 DB

## 记忆优化

### 记忆加入发言人标识
- **问题**：短期记忆只存 `{role, content}`，多人同时发言时 LLM 无法区分消息归属（如 A 问天气、B 打招呼，回复被混在一起且重复执行天气任务）
- **修复**：写入记忆的用户消息加上发言人标识 `[昵称(QQ:xxx) 在群xxx] `（群名片优先，回退昵称/QQ），当前消息与历史消息都带标识；聊天记录（审计）仍存原始文本
- 新增 `buildMemorySpeaker` / `senderDisplayName` 辅助函数 + 单元测试

### 同一会话消息批处理（解决并发重复执行/回复串味）
- **问题**：同一秒到达的多条消息各自触发完整 ReAct 循环，导致同一任务被重复执行（如群里有哪些人查了两遍）、回复互相串味（如"你能干什么"的回复混入群成员列表）
